...
<body>
    ...
    ?>
		...
		<form action="AthletesConfirmationController.php" method="POST">
			<div id="epreuveContainer" class="input-group mb-3">
  			  ...
  			  <select style="width:540px;height:40px" id="epreuve" name="epreuves[]">
  			  	<option value="" disabled selected>Choisir epreuves ...</option>
		  	  	<?php
						include_once 'index.php';
						$discipline = intval($_POST['discipline']);
						$query = "SELECT * FROM 
									epreuve AS  E, appartient AS  A
									WHERE A.discipline_id ='$discipline' AND A.epreuves_id = E.id ;";
						...
						foreach($result as $val){		
						 	echo '<option value="'.$val['id'].'"> '.$val['epreuve_name'].'</option> ';
						}
				?>
			  </select>
			</div>
	...
	<div class = "wrapper">
  		<button style="margin-top: 50px;margin-left: 170" type="submit" class="btn btn-primary" class ="center">Finir ajout d'athlete</button>
	</div>
		</form>		
	</div>
</body>